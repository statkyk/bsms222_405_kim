...
ind <- order(pop)
states[ind]
my_df <- data.frame(name=states[ind],rank=sort(rank(pop)))
my_df
```
...
```{r}
ind <- which(is.na(na_example)!=TRUE)
mean(na_example[ind])
...
```{r}
C <- 5/9 * (temp-32)
city_temps <- data.frame(name = city, fahrenheit = temp, celsius = C)
city_temps
```
...
length(murder_rate[murder_rate < average])
```
...
```{r}
c("MA","ME","MI","MO","MU") %in% murders$abb != TRUE
