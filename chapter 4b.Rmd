...
```{r}
ref_avg <- NHANES %>% filter(Gender=="female" & AgeDecade==" 20-29") %>% summarize(average = mean(BPSysAve,na.rm=TRUE)) %>% pull()
ref_avg
```
...
```{r}
NHANES %>% filter(Gender=="female" & AgeDecade==" 20-29") %>% summarize(min(BPSysAve,na.rm=TRUE)) %>% pull()
NHANES %>% filter(Gender=="female" & AgeDecade==" 20-29") %>% summarize(max(BPSysAve,na.rm=TRUE)) %>% pull()
```
...
```{r}
pander(NHANES %>% filter(Gender=="female") %>% group_by(AgeDecade) %>% summarize(average=mean(BPSysAve,na.rm=TRUE),standard_deviation=sd(BPSysAve,na.rm=TRUE)))
```

5. Repeat exercise 4 for males.

```{r}
pander(NHANES %>% filter(Gender=="male") %>% group_by(AgeDecade) %>% summarize(average=mean(BPSysAve,na.rm=TRUE),standard_deviation=sd(BPSysAve,na.rm=TRUE)))
```
...
```{r}
pander(NHANES %>% group_by(AgeDecade,Gender) %>% summarize(average=mean(BPSysAve,na.rm=TRUE),standard_deviation=sd(BPSysAve,na.rm=TRUE)))
```
...
```{r}
pander(NHANES %>% filter(Gender == "male" & AgeDecade == " 40-49") %>% group_by(Race1) %>% summarize(average_systolic_blood_pressure=mean(BPSysAve,na.rm=TRUE)) %>% arrange(average_systolic_blood_pressure))
```
...
df5 <- map_df(n,compute_s_n) %>% mutate(n=n,map_df(n,compute_s_n_2))
pander(df5[,c(2,1,3)])
```