...
```{r}
library(dplyr)
d %>% group_by(p.Protein) %>% count() %>% filter(n==1)
d %>% group_by(p.Protein) %>% count() %>% filter(n!=1)
```
...
```{r}
d %>% group_by(Classification,p.Protein) %>% count() %>% filter(n!=1)
```
...
```{r}
d %>% group_by(Classification)%>% count() %>% ungroup() %>% mutate(proportion = n / sum(n))
```

Then, you might be intrigued to whether females and males have different occurrence in each disorder. Let's check it. 

```{r}
d %>% filter(PatientSex=="M") %>% group_by(Classification) %>% count() %>% ungroup() %>% mutate(proportion = n / sum(n))
d %>% filter(PatientSex=="F") %>% group_by(Classification) %>% count() %>% ungroup() %>% mutate(proportion = n / sum(n))
```
...
```{r}
d %>% group_by(p.Protein,c.DNA) %>% count() %>% filter(n!=1)
```
