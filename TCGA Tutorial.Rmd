...
# Q1: What is this? This makes an empty space in the plot of lusc.
  ggplot(., aes(shortLetterCode, n, fill=shortLetterCode)) + 
  geom_bar(stat="identity", position=position_dodge()) + 
  facet_wrap(~type, scales = 'free_x', ncol=5) 
# Q2: What is this? This allows us to merge the plots.
```
...
Q: Do you think whether it is good visualization for the distribution? Yes, it is quite good visualization for the distribution.
...
```{r}
d_luad %>%
  ggplot(aes(gender,years_smoked)) +
  geom_violin() +
  labs(x="Gender",y="Smoking History",
       title="Smoking History by Gender")
```
...
```{r}
d_luad %>%
  ggplot(aes(gender,years_smoked,fill=gender)) +
  geom_violin() +
  labs(x="Gender",y="Smoking History",
       title="Smoking History by Gender")
```
...
q <- d_luad %>%
  ...
  labs(x="Gender",y="Smoking History",
       title="Smoking History by Gender")
ggsave('plot.TCGA_Tutorial.pdf', q)
```
